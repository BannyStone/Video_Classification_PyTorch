...
class GSV_ResNet3D(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, feat=False, **kwargs):
        ...
        self.inplanes = 64
        super(GSV_ResNet3D, self).__init__()
        self.feat = feat
...
class GSV_ResNet3D_lite(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, feat=False, **kwargs):
        if not isinstance(block, list):
            ...
            assert(len(block)) == 4, "Block number must be 4 for ResNet-Stype networks."
        self.inplanes = 64
        super(GSV_ResNet3D_lite, self).__init__()
        self.feat = feat
        self.conv1 = nn.Conv3d(3, 64, 
                               kernel_size=(1, 7, 7), 
                               stride=(1, 2, 2), 
...
def gsv_resnet50_3d_v3(pretrained=False, feat=False, **kwargs):
...
def gsv_resnet50_3d_v3_lite(pretrained=False, feat=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = GSV_ResNet3D_lite([GSVBottleneck3D_000, GSVBottleneck3D_100, GSVBottleneck3D_100, GSVBottleneck3D_100], 
                     [3, 4, 6, 3], feat=feat, **kwargs)
    if pretrained:
        if kwargs['pretrained_model'] is None:
            state_dict = model_zoo.load_url(model_urls['resnet50'])
