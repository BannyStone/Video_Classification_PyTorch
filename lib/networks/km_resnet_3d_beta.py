...
class TKMConv(nn.Module):
    def __init__(self, inplanes, planes, t_stride, temperature=1, init="zeros", std=0.01):
        super(TKMConv, self).__init__()
        ...
        self.conv_p = nn.Conv3d(inplanes, planes, 
                               kernel_size=(1, 1, 1), 
                               stride=(1, 1, 1),
                               padding=(0, 0, 0), 
                               bias=False)
    ...
    def forward(self, x):
        # generate mask
        mask = self.softmax(self.mask/self.temperature)
        # channel transformation
        out = self.conv_p(x)
        # temporal modeling
        out = F.conv3d(out, mask, None, (self.t_stride,1,1),
                        (1, 0, 0), (1, 1, 1), self.planes)
        return out

class TKMConvPre(nn.Module):
    def __init__(self, inplanes, planes, t_stride, temperature=1, init="zeros", std=0.01):
        super(TKMConv, self).__init__()
        assert(init in ("zeros", "ones", "normal")), "Invalid init method"
        self.temperature = temperature
...
def compute_tkmconv(m,x,y):
    x = x[0]
    cin = m.inplanes
    # batch_size = x.size(0)

    kernel_ops = m.conv_p.weight.size()[2:].numel()
    ops_per_element = kernel_ops
    output_elements = y.nelement()
    if m.t_stride == 1:
        mid_nelement = y.nelement()
    else:
        mid_nelement = y.nelement()*2

    # cout x oW x oH
    chatrans_ops = cin * mid_nelement * ops_per_element

    total_ops = output_elements * 3 + chatrans_ops
    m.total_ops = torch.Tensor([int(total_ops)])


class KMBottleneck3D_sample(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, 
                temperature=1, ratio=0.5, 
                stride=1, t_stride=1, downsample=None):
        super(KMBottleneck3D_sample, self).__init__()
        ...
        if t_channels != 0:
            ...
            self.conv1_t = TKMConv(inplanes, planes, t_stride=t_stride, temperature=temperature)
        if p_channels != 0:
...
class Bottleneck3D_KM(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, ratio=0.5, temperature=1, stride=1, t_stride=1, downsample=None):
        super(Bottleneck3D_KM, self).__init__()
        self.tkm_conv = TKMConv(inplanes, planes, t_stride=t_stride, temperature=temperature)
        self.bn1 = nn.BatchNorm3d(planes)
...
def km_resnet26_3d_v2_sample(pretrained=False, feat=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    ratios = (1, 1, 1, 1)
    model = PIBResNet3D_8fr([Bottleneck3D_KM, Bottleneck3D_KM, Bottleneck3D_KM, Bottleneck3D_KM], 
                     [2, 2, 2, 2], ratios, temperature=1/10, feat=feat, **kwargs)
    if pretrained:
        if kwargs['pretrained_model'] is None:
            state_dict = model_zoo.load_url(model_urls['resnet50'])
...
def km_resnet50_3d_v2_sample(pretrained=False, feat=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    ratios = (1, 1, 1, 1)
    model = PIBResNet3D_8fr([KMBottleneck3D_sample, KMBottleneck3D_sample, KMBottleneck3D_sample, KMBottleneck3D_sample], 
                     [3, 4, 6, 3], ratios, temperature=1/10, feat=feat, **kwargs)
    if pretrained:
        if kwargs['pretrained_model'] is None:
            state_dict = model_zoo.load_url(model_urls['resnet50'])
        else:
