...
from ..modules.fst import sharenormGSV as GSV
from ..modules.fst import sharenormGSV_spt as GSV_spt
...
class basicGSV(nn.Module):
    def __init__(self,
        ...
        self.gsv = GSV(inplanes, mid_planes, planes, 
                        s_kernel_size=s_kernel_size,
                        t_kernel_size=t_kernel_size,
                        s_stride=s_stride,
                        t_stride=t_stride,
...
class pureGSV(nn.Module):
    def __init__(self,
        ...
        self.gsv = GSV_spt(inplanes, mid_planes, planes, 
                        s_kernel_size=s_kernel_size,
                        t_kernel_size=t_kernel_size,
                        s_stride=s_stride,
                        t_stride=t_stride,
...
class GSVBlock(nn.Module):
    def __init__(self, inplanes, planes, s_stride=1, t_stride=1, downsample=None):
        super(GSVBlock, self).__init__()
        self.conv1 = basicGSV(inplanes, planes, 
                        s_kernel_size=3, t_kernel_size=3, 
                        s_stride=s_stride, t_stride=t_stride)
        self.conv2 = basicGSV(planes, planes, 
                        s_kernel_size=3, t_kernel_size=3)
        self.downsample = downsample
...
class pureGSVBlock(nn.Module):
    def __init__(self, inplanes, planes, s_stride=1, t_stride=1, downsample=None):
        super(pureGSVBlock, self).__init__()
        self.conv1 = pureGSV(inplanes, planes, 
                        s_kernel_size=3, t_kernel_size=3, 
                        s_stride=s_stride, t_stride=t_stride)
        self.conv2 = pureGSV(planes, planes, 
                        s_kernel_size=3, t_kernel_size=3)
        self.downsample = downsample
...
def pure_gsv_resnet18(pretrained=False, feat=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = FSTNet((pureGSVBlock,) * 4, [2, 2, 2, 2], feat=feat, **kwargs)
    if pretrained:
        if kwargs['pretrained_model'] is None:
            state_dict = model_zoo.load_url(model_urls['resnet50'])
        else:
...
def gsv_resnet18(pretrained=False, feat=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = FSTNet((pureGSVBlock, pureGSVBlock, GSVBlock, GSVBlock), [2, 2, 2, 2], feat=feat, **kwargs)
    if pretrained:
        if kwargs['pretrained_model'] is None:
            state_dict = model_zoo.load_url(model_urls['resnet50'])
        else:
